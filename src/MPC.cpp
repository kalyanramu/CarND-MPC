...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  //
  // 4 * 10 + 2 * 9
  // N time steps for each variable in state (variables in state = 6)
  // N-1 time steps for each variable in control inputs (varibales in ctrl input = 2)
  size_t n_vars = N * 6 + (N-1) *2 ; 

  // TODO: Set the number of constraints
  // There is predictive equation for each state variable (variables in state =6)
  ...
  for (unsigned int i = 0; i < delta_start; i++) {
    vars_lowerbound[i] = -MAX_VAL;
    vars_upperbound[i] = MAX_VAL;
  }
